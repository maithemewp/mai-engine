...
## 2.41.0 (TBD)
* Added: The Widgets screen can switch between blocks and the classic screen. New sites use blocks. A site that already has classic widgets keeps the classic screen and gets a notice with a "Switch to blocks" button and a "Hide forever" link. The switch is always in the Widgets screen's Help menu too. The block screen always shows a pinned link back to classic. Existing widgets stay as they are and show as Legacy Widget blocks in the block editor. Genesis's notice about adding a code snippet is removed. [Developers] The choice runs on `use_widgets_block_editor` at priority 5, so a value set at the default priority 10 still wins.
